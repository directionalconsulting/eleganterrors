...
$current_dir = str_replace($scriptname, '', $docroot);

$host = $_SERVER['HTTP_HOST'];
$request = $_SERVER['REQUEST_URI'];

$hostpath = $host.str_replace($scriptname, '', $request);
/* $base = 'http://'.$hostpath;
$base = substr($base, 0, (strrchr($base, "/") -1));
 */
$base = "http://$host";

if (_REMOTE == 'TRUE') { $base = 'http://www.sitereviewboard'; }

define('BASE_PATH', $base);
$basepath = $smarty->_tpl_vars['base'];
...
function smarty_imagetext($params)
    ...
    ### Preferences
    // Read config file
    if (empty($params['textcache'])) SMARTY_DIR.$textcache_root = 'textcache';
    ...
    $config['main']['cache']    = "$textcache_root/cache/"; // Nur dieser Ordner muss �ffentlich zug�nglich sein
    $config['main']['http']    = BASE_PATH."/$textcache_root/cache/"; // Nur dieser Ordner muss �ffentlich zug�nglich sein
    ...
    // Error handling
    if (empty($params['text']) && !isset($params['text'])) { $smarty->trigger_error("imagetext: missing 'text' parameter"); return; }
    if (empty($params['style']) && !isset($params['style'])) { $smarty->trigger_error("imagetext: missing 'style' parameter"); return; }
    if (empty($style['font']) && !isset($params['style'])) { $smarty->trigger_error("imagetext: missing 'font' parameter"); return; }
    if (empty($style['size']) && !isset($style['size'])) { $smarty->trigger_error("imagetext: missing 'size' parameter"); return; }
    if (empty($style['bgcolor']) && !isset($style['bgcolor'])) { $style['bgcolor'] = 'FFFFFF'; }
    if (empty($style['fgcolor']) && !isset($style['fgcolor'])) { $style['fgcolor'] = '000000';  }
    ...
    $imgurl = $config['main']['cache'].$params['style'].'_'.$hash.'.'.$ext;
    $httpurl = $config['main']['http'].$params['style'].'_'.$hash.'.'.$ext;
    // The url is changing on dynamic images
    if ($style['dynamic'] OR $style['dev'])
        {
        $hash = md5(implode('',$style).$smarty->imagetextcount);
        $imgurl = $config['main']['cache'].$smarty->imagetextcount.'_'.$hash.'.'.$ext;
        $httpurl = $config['main']['http'].$smarty->imagetextcount.'_'.$hash.'.'.$ext;
    ...
    if (file_exists($imgurl) && $style['dev'] != true && $style['dynamic'] != true)
          ...
          if ( empty($style['html']) ) return $style['prehtml'].'<img src="'.$httpurl.'" border="0" alt="'.preg_replace("=\r\n|\r|\n|\t=", ' ', htmlspecialchars($params['text'])).'" '.$style['html'].' />'.$style['posthtml'];
          else return $style['prehtml'].'<img src="'.$httpurl.'" border="0" '.$style['html'].' />'.$style['posthtml'];
    ...
    if ($style['urlonly'] == 'true') return $httpurl;
        else
        if ( empty($style['html']) ) return $style['prehtml'].'<img src="'.$httpurl.'" style="border: '.$border.'px solid #00ff00" alt="'.preg_replace("=\r\n|\r|\n|\t=", ' ', htmlspecialchars($params['text'])).'" '.$style['html'].' />'.$style['posthtml'];
        else return $style['prehtml'].'<img src="'.$httpurl.'" style="border: '.$border.'px solid #00ff00" '.$style['html'].' />'.$style['posthtml'];
    }
?>